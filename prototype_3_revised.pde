...
int lastActionTime = -1000000;
final int MIN_ACTION_GAP = 100;

int numRepeats = 1; //sets the number of times each button repeats in the test

int lastClickedId = -1;
int flashStartTime = 0;      
int hitFlashDuration = 150;
...
void mousePressed()
{
  // block rapid consecutive actions
  if (millis() - lastActionTime < MIN_ACTION_GAP) return;
  ...
  int clickedId = trials.get(trialNum);
...
void drawButton(int i)
{
  ...
  boolean flashing = (millis() - flashStartTime < hitFlashDuration);
  boolean isLastClicked = (i == lastClickedId);
  ...
  if (isLastClicked && flashing && lastWasHit) {
    fill(120, 255, 120); 
  }
  else if (isCurrent) {
    fill(255, 90, 90);
...
void keyPressed(){
  if (key == ' ')
  {
    if (spaceLatch) return;      // prevents OS key repeat
    spaceLatch = true;

    // block rapid consecutive actions (space presses too)
    if (millis() - lastActionTime < MIN_ACTION_GAP) return;
    lastActionTime = millis();
